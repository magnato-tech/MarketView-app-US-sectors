...
const PersistentTooltip = ({ point, onClose }: any) => {
  return (
    <div className="bg-slate-900/95 backdrop-blur-md border border-slate-700 p-3 rounded-lg shadow-2xl text-xs absolute" style={{ top: '10px', left: '10px' }}>
      <p className="font-bold text-slate-300 mb-2 border-b border-slate-800 pb-1">{point.timestamp}</p>
      <div className="space-y-1.5">
        {Object.keys(point).map((key) => {
          if (key === 'timestamp') return null;
          const ticker = TICKERS.find(t => t.symbol === key);
          if (!ticker) return null;
          return (
            <div key={key} className="flex items-center justify-between gap-6">
              <div className="flex items-center gap-2">
                <div className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: ticker.color }}></div>
                <span className="text-slate-400">{ticker.name}:</span>
              </div>
              <span className={`font-mono font-bold ${point[key] >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                {point[key] > 0 ? '+' : ''}{point[key]}%
              </span>
            </div>
          );
        })}
      </div>
      <button onClick={onClose} className="absolute top-1 right-1 text-slate-300 hover:text-slate-200">X</button>
    </div>
  );
};
...
const MainDashboard: React.FC<DashboardProps> = ({ 
  ...
  const [lockedPoints, setLockedPoints] = useState<any[]>([]);

  const handlePointClick = (data: any) => {
    if (lockedPoints.length >= 3) return;
    setLockedPoints([...lockedPoints, data]);
  };

  const handlePersistentTooltipClose = (timestamp: string) => {
    setLockedPoints(lockedPoints.filter(point => point.timestamp !== timestamp));
  };
        ...
        <div className="flex-1 min-h-0 min-w-0 flex flex-col bg-slate-900 border border-slate-800 rounded-2xl p-4 shadow-xl overflow-hidden">
          ...
          <div className="flex-1 min-h-0 w-full relative">
            ...
            {lockedPoints.map(point => (
              <PersistentTooltip key={point.timestamp} point={point} onClose={() => handlePersistentTooltipClose(point.timestamp)} />
            ))}
          </div>
        </div>
      </div>
